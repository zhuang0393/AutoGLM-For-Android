...
| 权限获取 | ADB shell 命令      | System Privileges (系统权限) |
| 文本输入 | ADB Keyboard        | 内置 AutoGLM Keyboard        |
| 用户界面 | 命令行              | 原生 Android UI + 悬浮窗     |
| 截图方式 | ADB screencap       | System API                   |
...
### 第一步：安装 AutoGLM For Android

此处假设您已经编译生成了 `AutoGLM` 系统应用，并集成到了 ROM 中，或者拥有系统签名的 APK。
...
- 通过 Instrumentation/Shell 执行操作
- 实现点击、滑动、按键等操作
- 支持人性化滑动轨迹

**ScreenshotService (screenshot/ScreenshotService.kt)**

- 截取屏幕并压缩为 WebP (使用系统 API)
